...
SYSTEM_POS_ROOT="$PROJECT_ROOT/src/system-pos"
cd "$PROJECT_ROOT"
...
check_database() {
    ...
    if (cd "$SYSTEM_POS_ROOT/apps/api" && npx tsx -e "
...
start_api() {
    ...
    if [ ! -d "$SYSTEM_POS_ROOT/apps/api" ]; then
        print_error "API directory not found: $SYSTEM_POS_ROOT/apps/api"
        return 1
    fi
...
start_admin() {
    ...
    if [ ! -d "$SYSTEM_POS_ROOT/apps/admin" ]; then
        print_error "Admin directory not found: $SYSTEM_POS_ROOT/apps/admin"
        return 1
    fi
...
start_mobile() {
    ...
    if [ ! -d "$SYSTEM_POS_ROOT/apps/mobile" ]; then
        print_error "Mobile directory not found: $SYSTEM_POS_ROOT/apps/mobile"
        return 1
    fi
    ...
    cd "$SYSTEM_POS_ROOT/apps/mobile"
